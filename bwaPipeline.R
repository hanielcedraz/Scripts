...
if (!opt$singleEnd) {
    mappingList <- function(samples, reads_folder, column){
        ...
        for (i in 1:nrow(samples)) {
            reads <- dir(path = file.path(reads_folder), pattern = c("fastq.gz$", "fq.gz$"), full.names = TRUE)
    ...
    mappingList <- function(samples, reads_folder, column){
        mapping_list <- list()
        for (i in 1:nrow(samples)) {
            reads <- list.files(path = file.path(reads_folder), pattern = "fastq.gz$|fq.gz$", full.names = TRUE)
            #reads <- dir(path = file.path(reads_folder), pattern = c"fastq.gz$|fq.gz$", full.names = TRUE)
...
index_Folder <- paste(dirname(mappingTarget), '/', 'index_BWA', '/', sep = '')
    ...
    hisat2.single.mapping <- mclapply(mapping, function(index){
        ...
        try({
            system(paste('bwa mem',
                         ...
                         opt$mappingTarget,
                         #paste0(index_Folder,index_names),
                         paste0(index$SE, ">", mapping_Folder, '/', index$sampleName, '_unsorted_aligned.sam'),
                         if (file.exists(external_parameters)) line))})
    }, mc.cores = opt$mprocs
    )
...
samtoolsList <- function(samples, reads_folder, column){
    ...
    for (i in 1:nrow(samples)) {
        ...
        maps <- lapply(c("_unsorted_aligned.sam"), grep, x = samfiles, value = TRUE)
