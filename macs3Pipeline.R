...
#setwd("/Users/haniel/Documents/macs3pipeline")
...
option_list <- list(
    ...
    make_option(c('-k', '--callpeak'), type = 'logical', default = TRUE, help = "Main MACS3 Function: Call peaks from alignment results. [default %default",
                dest = 'callpeak'),
    make_option(c("-c", "--column"), type = "character", default = "SAMPLE_ID", help = "Column name from the sample sheet to use as sample names [default %default]",
                dest = "column"),
...
if (opt$callpeak) {
    ...
    if (!all(sapply(macs.run, "==", 0L))) {
        write(paste("Something went wrong with macs2. Some jobs failed"),stderr())
        stop()
    } else {
    ...
    if (!all(sapply(macs.run, "==", 0L))) {
        ...
        write(paste("You did not give any parameter for running macs2. Please provide the file containing the parameters for analysis"), stderr())
    }
}
